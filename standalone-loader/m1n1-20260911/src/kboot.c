...
/* Private opt-in: use only the original firmware-provided J714s framebuffer.
 * No allocation, mode change or display-register write. Default stays cropped.
 */
static bool azahi_full_height_fb_ok(u64 cropped_height)
{
    return chip_id == T6050 && board_id == 8 && cur_boot_args.video.base &&
           cur_boot_args.video.width == 3024 && cur_boot_args.video.height == 1964 &&
           cur_boot_args.video.stride == 12096 && cropped_height == 1890 &&
           ((cur_boot_args.video.depth & 0xff) == 30 ||
            (cur_boot_args.video.depth & 0xff) == 32);
}
...
static int dt_set_fb(void)
{
    ...
    int full_len = 0;
    const fdt32_t *full = fdt_getprop(dt, chosen, "azahi,full-height-framebuffer", &full_len);
    if (full && full_len == sizeof(*full) && fdt32_to_cpu(*full) == 1 &&
        !fdt_node_check_compatible(dt, 0, "apple,j714s") && azahi_full_height_fb_ok(fb_height)) {
        fb_base = cur_boot_args.video.base;
        fb_height = cur_boot_args.video.height;
        printf("AZAHI_FULL_HEIGHT_FB: original %lux%lu, notch %lu rows\n",
               cur_boot_args.video.width, fb_height, notch_height);
    } else if (full) {
        printf("AZAHI_FULL_HEIGHT_FB: guard refused; retaining cropped framebuffer\n");
    }
...
static int dt_set_memory(void)
{
    ...
    /*
     * T6050 bring-up: m1n1 has no MCC support for this SoC
     * ("MCC: Unsupported version:mcc,t6050"), so the protected carveouts are
     * never unmapped and the range above still contains regions that fault on
     * access. Probing over the proxy SErrors the machine at 0x10004400000,
     * which is only ~9 MB above phys_base — so any OS that allocates memory
     * dies instantly, before it can print anything. A 132-byte payload that
     * touches only the framebuffer runs fine; U-Boot and Linux both die.
     *
     * Fence the OS off from the bottom of RAM until carveouts are handled.
     */
    if (chip_id == T6050 || chip_id == T6051) {
        /*
         * Verified against /chosen/carveout-memory-map in this machine's ADT:
         * a 1764 MiB protected block sits at 0x10004000000..0x10072400000,
         * i.e. only 5 MiB above phys_base, and reading into it (0x10004400000,
         * region-id-45) SErrors the machine. That is why every payload which
         * allocates RAM dies instantly while a framebuffer-only payload lives.
         * 0x1010A960000..0x10F4AB00000 is the largest carveout-free span (57 GiB).
         */
        /*
         * Start ABOVE the region the hypervisor loads the guest image into.
         * hv.load_raw() places the guest at u.heap_top: a 4.24 GB KDE image
         * landed at 0x10018984000..0x10115B28000, overlapping the old
         * 0x1010A960000 start by ~180 MB, and the machine died the instant the
         * guest was entered (CDC link dropped, zero guest console bytes) while
         * the 980 MB image booted fine from the same base. 0x10200000000 is
        ...
        if (dram_min < safe_min || dram_max > safe_max) {
...
{
    /*
     * T6050 bring-up: the payload - and therefore the initrd unpacked from it -
     * lives in m1n1's heap, which sits BELOW the carveout-free window we
     * advertise to the OS (see dt_set_memory: 0x1010A960000..0x10F4AB00000).
     * Linux checks the initrd against the linear mapping, finds it outside,
     * and silently discards it:
     *   "initrd not fully accessible via the linear mapping -- please check
     *    your bootloader"
     * then panics with no root filesystem. Copy it into the safe window,
     * clear of the kernel (0x10800000000) and the relocated FDT
     * (0x10900000000).
     */
    if ((chip_id == T6050 || chip_id == T6051) && start && size) {
        void *safe = (void *)0x10A00000000UL;
        printf("T6050: relocating initrd %p -> %p (0x%zx bytes)\n", start, safe, size);
        memcpy(safe, start, size);
        start = safe;
    }
...
int kboot_prepare_dt(void *fdt)
{
    ...
    /* Reserve the whole bounded high-RAM test loader/payload/heap window. */
    u64 loader_reserved = (u64)_base == 0x10400000000UL ? 0x10000000UL :
                          ((u64)_end) - ((u64)_base);
    if (fdt_add_mem_rsv(dt, (u64)_base, loader_reserved))
...
int kboot_boot(void *kernel)
{
    ...
    usb_init();
    /*
     * T6050 (M5 Pro) bring-up: pcie_init() has no apcie support for this SoC
     * (prints "Unsupported compatible"), and dapf_init_all() writes address
     * filter registers for a fixed list of devices with no chip gating at all
     * — on T6050 that faults with an asynchronous SError (L2C_ERR_STS 0x82)
     * before we ever reach the kernel. Skip both until they're implemented.
     */
    if (chip_id == T6050 || chip_id == T6051) {
    ...
    /*
     * T6050: m1n1's heap lives in the protected block, so the FDT would sit in
     * memory the OS is not allowed to touch. Copy it into the carveout-free
     * window (kernel is forced to 0x10800000000; keep well clear of it).
     */
    if (chip_id == T6050 || chip_id == T6051) {
        void *safe_dt = (void *)0x10900000000UL;
        size_t dt_size = fdt_totalsize(dt);
        printf("T6050: relocating FDT %p -> %p (0x%zx bytes)\n", dt, safe_dt, dt_size);
        memcpy(safe_dt, dt, dt_size);
        dt = safe_dt;
    }

...
{
	int ret, ret_reset;
	bool first_fixed_host = appledwc->state == DWC3_APPLE_PROBE_PENDING &&
		device_property_read_bool(appledwc->dev, "azahi,force-host-mode");
...
core_exit:
	if (first_fixed_host) {
		/*
		 * core_probe succeeded but no role was successfully initialized.
		 * 7.0.13 core_remove calls exit_mode AND core_exit. Avoid a second
		 * PHY exit and do not host_exit the failed host_init's freed xhci.
		 */
...
static int dwc3_apple_probe(struct platform_device *pdev)
{
	 ...
	 */
	appledwc->state = DWC3_APPLE_PROBE_PENDING;
	/* A fixed-host experiment has no role-switch producer. Do not publish
	 * callbacks pointing at this devm allocation before forced init can fail.
	 */
...
static void dwc3_apple_remove(struct platform_device *pdev)
{
	...
	if (device_property_read_bool(&pdev->dev, "azahi,force-host-mode")) {
		/* Fixed-host success never traversed a cable disconnect/core_exit.
		 * Let core_remove tear down the host and PHY exactly once.
		 */
		if (appledwc->state == DWC3_APPLE_HOST)
			dwc3_core_remove(&appledwc->dwc);
		reset_control_assert(appledwc->reset);
		return;
	}

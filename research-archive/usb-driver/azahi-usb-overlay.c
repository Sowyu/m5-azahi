...
MODULE_PARM_DESC(dry_run, "Only report PMGR power state; do not read peripheral banks or apply an overlay");
...
static const struct ps_desc ps_descs[] = {
	...
	{ "ATC2_PHYMXWRAP", PMGR2_BASE, PS_ATC2_PHYMXWRAP, true },
};
...
static bool preflight_pmgr(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(ps_descs); i++) {
		...
		if (ps_descs[i].required &&
		    (PS_ACTUAL(v) != PS_ACTIVE || PS_TARGET(v) != PS_ACTIVE))
			all_active = false;
	}
	return all_active;
}
...
static int preflight_blocks(void)
{
	bool ok = true;
	...
	pr_info("azahi-usb-overlay: PIPEHANDLER mux_ctrl %#x aon_gen %#x nonselected_override %#x (loader: 0x22 / 0x1 / 0x9332)\n",
		mux, aon_gen, nonsel);
	pr_info("azahi-usb-overlay: DWC3 GSNPSID %#x\n", snpsid);
	/* Match the IP families accepted by this exact 7.0.13 DWC3 core. */
	if ((snpsid >> 16) != 0x5533 && (snpsid >> 16) != 0x3331 &&
	    (snpsid >> 16) != 0x3332) {
		pr_err("azahi-usb-overlay: unexpected DWC3 ID; refusing overlay\n");
		return -ENODEV;
	}
	return 0;
}
...
static int check_live_tree(void)
{
	...
	static const char * const new_nodes[] = {
		USB_NODE_PATH, "/soc/phy@382a90000",
		"/soc/iommu@382f00000", "/soc/iommu@382f80000",
	};
	int i;
...
static int __init azahi_usb_overlay_init(void)
{
	...
	/* Do not use unvalidated genpd nodes to turn a failed gate check into MMIO. */
	if (use_pmgr) {
		pr_err("azahi-usb-overlay: PMGR variant withheld pending power/reset audit\n");
		return -EOPNOTSUPP;
	}
	if (!active) {
		pr_err("azahi-usb-overlay: refusing overlay with inactive domains (no force bypass)\n");
		return -ENODEV;
	}
	ret = preflight_blocks();
	if (ret)
		return ret;

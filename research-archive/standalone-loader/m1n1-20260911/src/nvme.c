...
{
    /*
     * T6050 bring-up: m1n1's own NVMe driver must NEVER run on this SoC.
     * M4+/T8132 split the register file - NVMe ops moved to ans reg[9]
     * (0x45dcc0000) while reg[3] became NVMMU only - so this driver, which
     * still uses the reg[3] base, touches registers that do not decode.
     * Worse, when it then fails it calls nvme_ctrl_shutdown(), whose
     * NVME_CSTS poll loop SErrors on EVERY iteration: m1n1 dumps registers,
     * recovers via GUARD_MARK, re-faults, forever. The screen scrolls
     * register dumps with nothing listening and the machine needs a PHYSICAL
     * power cycle (there is no remote reset - USB-C VDM is rejected here).
     * That burned several of the operator's power cycles.
     *
     * Linux's apple-nvme drives the SSD instead; it knows the T8132 layout
     * and its RTKit handshake has proper timeouts, unlike src/rtkit.c:637.
     */
    if (chip_id == T6050 || chip_id == T6051) {
        printf("nvme: T6050 bring-up: m1n1's NVMe driver is DISABLED "
               "(wrong register base for this SoC; Linux drives the SSD)\n");
        return false;
    }

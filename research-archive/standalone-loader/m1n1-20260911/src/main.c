...
void run_actions(void)
{
    ...
    /*
     * T6050 bring-up: keep the framebuffer console ON across the payload boot.
     * Upstream only calls fb_set_active(true) in the "no payload" path, so a
     * successful payload boot shows the logo and nothing else — which makes an
     * m1n1 crash indistinguishable from a kernel hang. We want to see every
     * message up to "Preparing to boot kernel".
     */
    fb_set_active(true);
...
void m1n1_main(void)
{
    ...
    heapblock_init();
    /* Bound the high-RAM test loader inside its explicit 256 MiB reservation. */
    if ((u64)_base == 0x10400000000UL)
        heapblock_set_limit((void *)0x10410000000UL);

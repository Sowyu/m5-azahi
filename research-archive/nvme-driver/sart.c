...
/* J714s SARTv4, same first 16 entries used by the verified HV translation.
 * Entries beyond 15 are never accessed or changed by this diagnostic.
 */
static void sart4_get_entry(struct apple_sart *sart, int index, u8 *flags,
			    phys_addr_t *paddr, size_t *size)
{
	*paddr = (phys_addr_t)readl(sart->regs + 0x60 + 4 * index) << 12;
	*size = (size_t)readl(sart->regs + 0xc0 + 4 * index) << 12;
	*flags = readl(sart->regs + 4 * index);
}

static void sart4_set_entry(struct apple_sart *sart, int index, u8 flags,
			    phys_addr_t paddr_shifted, size_t size_shifted)
{
	writel(paddr_shifted, sart->regs + 0x60 + 4 * index);
	writel(size_shifted, sart->regs + 0xc0 + 4 * index);
	writel(flags, sart->regs + 4 * index);
}

static struct apple_sart_ops sart_ops_j714s = {
	.get_entry = sart4_get_entry,
	.set_entry = sart4_set_entry,
	.flags_allow = APPLE_SART3_FLAGS_ALLOW,
	.size_shift = APPLE_SART3_SIZE_SHIFT,
	.paddr_shift = APPLE_SART3_PADDR_SHIFT,
	.size_max = APPLE_SART3_SIZE_MAX,
...
static const struct of_device_id apple_sart_of_match[] = {
	{
		.compatible = "azahi,j714s-sart-v4",
		.data = &sart_ops_j714s,
	},
	{

 ...
 *
 * azahi fork for Apple T6050 (M5 Pro), J714s. Private one-machine bring-up
 * code, not for upstream. Diff against vendor/pcie-apple.original.c. Deltas:
 *   - distinct compatible "azahi,t6050-pcie" only, so it binds deliberately
 *     and never competes with the built-in stock driver (CONFIG_PCIE_APPLE=y);
 *     the overlay node ships disabled until the loader bring-up succeeds;
 *   - t6050_hw is the t602x register layout, confirmed against the macOS
 *     AppleT6050PCIe kext (build 26A428): port PERST 0x82c, RID2SID 0x3000,
 *     MSIMAP 0x3800, MSIADDR 0x16c/0x170;
 *   - on a link-training timeout, dump the port state registers so the first
 *     attended hardware session can see why the link did not come up.
 * This driver adopts a link the loader already trained (like the stock driver
 * does on M4); it does not do the controller/PHY bring-up itself. That is in
 * standalone-loader/.../azahi_pcie.c, which must run first.
 */
...
/*
 * T6050 (M5 Pro) uses the t602x port register layout (confirmed against the
 * macOS AppleT6050PCIe kext, build 26A428: PERST 0x82c, RID2SID 0x3000,
 * MSIMAP 0x3800). The RID2SID table has 64 entries on this SoC; the probe
 * autodetects the real count via the RAZ/WI check, so the 512 upper bound is
 * harmless. Kept as a named struct so T6050-only deltas have a home.
 */
static const struct hw_info t6050_hw = {
	.phy_lane_ctl		= 0,
				 ...
				 struct device_node *np)
{
	...
	if (!(link_stat & PORT_LINKSTS_UP)) {
		...
		if (!left) {
			...
			/*
			 * First-session diagnostic: with no hardware history for
			 * this SoC, log the port state so a stuck link can be
			 * told apart from a device that never asserted PERST or
			 * a loader that skipped the controller bring-up.
			 */
			dev_warn(pcie->dev,
				 "%pOF LINKSTS %#x STATUS %#x LINKCMDSTS %#x INTSTAT %#x APPCLK %#x\n",
				 np,
				 readl_relaxed(port->base + PORT_LINKSTS),
				 readl_relaxed(port->base + PORT_STATUS),
				 readl_relaxed(port->base + PORT_LINKCMDSTS),
				 readl_relaxed(port->base + PORT_INTSTAT),
...
static const struct of_device_id apple_pcie_of_match[] = {
	/* azahi only: the built-in stock driver keeps the stock compatibles. */
	{ .compatible = "azahi,t6050-pcie",	.data = &t6050_hw },
	{ }
};
...
MODULE_DESCRIPTION("Apple T6050 PCIe host bridge driver (azahi fork)");

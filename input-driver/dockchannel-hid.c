...
static int dchid_reset_interface(struct dchid_iface *iface, int state)
{
	...
	u8 v2[] = { CMD_RESET_INTERFACE, 2, iface->index, state, 0, 0, 0, 0, 0 };
	int ret;

	/* Local J714s bring-up: v2 interface-power will/has pair. Protocol
	 * derived by Project Wallace for J614s; do not change older boards.
	 * This is power report 0x40, despite the historical function name.
	 */
	if (of_machine_is_compatible("apple,j714s")) {
		ret = dchid_comm_cmd(iface->dchid, v2, sizeof(v2));
		if (ret < 0)
			return ret;
		v2[4] = 1;
		ret = dchid_comm_cmd(iface->dchid, v2, sizeof(v2));
		if (ret >= 0)
			dev_info(iface->dchid->dev,
				 "AZAHI_V2_POWER iface=%d state=%d accepted\n",
				 iface->index, state);
		return ret;
	}

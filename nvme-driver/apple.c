...
#ifdef AZAHI_ROOT_WRITES
#include "root-write-policy.h"

/* No writable range knobs. Arming at module-load time is rejected; the
 * initramfs must first probe read-only and validate GPT + filesystem identity.
 * This is an accidental-write guard, not a security boundary against root.
 */
static bool root_write_armed;
static bool rootguard_bound;

static int root_write_arm_set(const char *val, const struct kernel_param *kp)
{
	bool enable;
	int ret = kstrtobool(val, &enable);
	if (ret)
		return ret;
	if (enable && !READ_ONCE(rootguard_bound))
		return -EPERM;
	WRITE_ONCE(root_write_armed, enable);
	pr_warn("J714S_ROOTGUARD writes %s; fixed root partition only\n",
		enable ? "armed" : "disarmed");
	return 0;
}

static const struct kernel_param_ops root_write_arm_ops = {
	.set = root_write_arm_set,
	.get = param_get_bool,
};
module_param_cb(root_write_armed, &root_write_arm_ops, &root_write_armed, 0600);
MODULE_PARM_DESC(root_write_armed, "Default off; enable only after GPT/root validation");

static bool apple_nvme_root_io_allowed(struct nvme_ns *ns,
		struct request *req, const struct nvme_command *cmd)
{
	struct azahi_root_io io;
	if (!ns || !ns->head)
		return false;
	io = (struct azahi_root_io) {
		.armed = READ_ONCE(root_write_armed),
		.passthrough = blk_rq_is_passthrough(req),
		.request = req_op(req) == REQ_OP_WRITE ? AZAHI_WRITE :
			   req_op(req) == REQ_OP_FLUSH ? AZAHI_FLUSH : AZAHI_OTHER,
		.nsid = ns->head->ns_id,
		.command_nsid = le32_to_cpu(cmd->rw.nsid),
		.lba_shift = ns->head->lba_shift,
		.metadata_size = ns->head->ms,
		.opcode = cmd->rw.opcode,
		.flags = cmd->rw.flags,
		.cdw2 = le32_to_cpu(cmd->rw.cdw2),
		.cdw3 = le32_to_cpu(cmd->rw.cdw3),
		...
		.apptag = le16_to_cpu(cmd->rw.lbat),
		.appmask = le16_to_cpu(cmd->rw.lbatm),
		.bytes = blk_rq_bytes(req),
	};
	return azahi_root_io_allowed(&io);
}
#endif
...
static int apple_nvme_create_cq(struct apple_nvme *anv)
{
	...
	/* Match the verified HV handoff before issuing CreateCQ. */
	if (anv->hw->j714s_readonly)
		writeq(anv->ioq.cq_dma_addr, anv->mmio_nvme + APPLE_ANS_IOQ_CQES);
...
static int apple_nvme_create_sq(struct apple_nvme *anv)
{
	...
	if (anv->hw->j714s_readonly)
		writeq(anv->ioq.sq_dma_addr, anv->mmio_nvme + APPLE_ANS_IOQ_CMDS);
					...
					const struct blk_mq_queue_data *bd)
{
	...
	/* Diagnostic build: no namespace writes, discard or vendor commands.
	 * This is enforced before mapping/submitting, independent of BLKROSET.
	 * Administrative setup is limited to non-media-mutating operations.
	 */
	if (anv->hw->j714s_readonly) {
		bool allowed = !q->is_adminq && cmnd->common.opcode == nvme_cmd_read;
#ifdef AZAHI_ROOT_WRITES
		if (!q->is_adminq && !allowed)
			allowed = apple_nvme_root_io_allowed(ns, req, cmnd);
#endif
		if (q->is_adminq) {
			switch (cmnd->common.opcode) {
			...
			case nvme_admin_set_features:
				...
				allowed = !(le32_to_cpu(cmnd->features.fid) & BIT(31));
				break;
			}
		}
		if (!allowed) {
			dev_warn_ratelimited(anv->dev, "J714S_IO_GUARD blocked opcode %02x admin=%d\n",
				cmnd->common.opcode, q->is_adminq);
			ret = BLK_STS_IOERR;
			goto out_free_cmd;
		}
	}
...
static void apple_nvme_reset_work(struct work_struct *work)
{
	...
	if (!(readl(anv->mmio_coproc + APPLE_ANS_COPROC_CPU_CONTROL) &
		...
		if (anv->hw->j714s_readonly) {
			dev_err(anv->dev, "J714S_RO requires running firmware; refusing power reset\n");
			ret = -EIO;
			goto out;
		}
	...
	if (anv->hw->has_lsq_nvmmu) {
		...
		writel(limit | (limit << 16), anv->mmio_nvme
...
static struct apple_nvme *apple_nvme_alloc(struct platform_device *pdev)
{
	...
	if (anv->hw->j714s_readonly && !of_machine_is_compatible("apple,j714s")) {
		ret = -ENODEV;
		goto put_dev;
	}
#ifdef AZAHI_ROOT_WRITES
	{
		const char *uuid;
		u64 first, end;
		if (of_property_read_string(dev->of_node, "azahi,root-partuuid", &uuid) ||
		    strcmp(uuid, AZAHI_ROOT_PARTUUID) ||
		    of_property_read_u64(dev->of_node, "azahi,root-first-lba", &first) ||
		    of_property_read_u64(dev->of_node, "azahi,root-end-lba", &end) ||
		    first != AZAHI_ROOT_FIRST_LBA || end != AZAHI_ROOT_END_LBA) {
			ret = -ENODEV;
			dev_err(dev, "J714S_ROOTGUARD missing/mismatched fixed partition binding\n");
			goto put_dev;
		}
	}
#endif
	...
	if (anv->hw->j714s_readonly) {
		anv->mmio_nvmmu = devm_platform_ioremap_resource_byname(pdev, "nvmmu");
		if (IS_ERR(anv->mmio_nvmmu)) {
			ret = PTR_ERR(anv->mmio_nvmmu);
			goto put_dev;
		}
		dev_info(dev, "J714S_RO native NVMe: split BAR, queue limit 63, media writes blocked\n");
	} else {
	...
	/* Diagnostic DT deliberately has no power/reset providers. */
...
#ifdef AZAHI_ROOT_WRITES
	WRITE_ONCE(root_write_armed, false);
	WRITE_ONCE(rootguard_bound, true);
	dev_warn(anv->dev, "J714S_ROOTGUARD read-only until explicitly armed; root=%s\n",
		 AZAHI_ROOT_PARTUUID);
#endif
...
#ifdef AZAHI_ROOT_WRITES
	WRITE_ONCE(rootguard_bound, false);
	WRITE_ONCE(root_write_armed, false);
#endif
...
static const struct apple_nvme_hw apple_nvme_j714s_ro_hw = {
	.has_lsq_nvmmu = true,
	.j714s_readonly = true,
	.max_queue_depth = 64,
};
...
#ifdef AZAHI_ROOT_WRITES
	{ .compatible = "azahi,j714s-nvme-rootguard", .data = &apple_nvme_j714s_ro_hw },
#else
	{ .compatible = "azahi,j714s-nvme-readonly", .data = &apple_nvme_j714s_ro_hw },
